...
.EXAMPLE
    ...
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/1nFlight/ODC-Reduced/main/Collect-IntuneLogs.ps1))) -Deep

.NOTES
    Requires administrator elevation.
    Changelog:
        1.1.1  2026-04-22  Fix repeated auth prompts on CurrentUser\My collection.
                           - `certutil -user -store My` acquires a crypt context
                             per certificate. For Windows Hello / TPM-backed /
                             smart-card-backed certs, that triggers a PIN or
                             Hello prompt *per cert*. On a device with N such
                             certs, the user sees N prompts.
                           - Replaced with Get-ChildItem Cert:\CurrentUser\My
                             (via encoded PS command inside the user-context
                             scheduled task). The cert provider reads public
                             metadata only (Subject, Issuer, thumbprint,
                             HasPrivateKey flag) without ever acquiring a
                             private key, so no prompts fire.
                           - Bumped timeout to 60s for users with many certs.
...
Invoke-Safe 'certificates (CurrentUser\My) - interactive user context' {
    # IMPORTANT: do NOT use certutil here. `certutil -user -store My` calls
    # CryptAcquireContext on every cert it enumerates. For Windows Hello /
    # TPM-backed / smart-card-backed certs, that pops a PIN or Hello prompt
    # PER CERT — on a typical enrolled device with 3-5 WHfB + SCEP certs
    # the user sees 3-5 consecutive prompts during collection.
    #
    # PS cert provider reads metadata only (does not acquire private keys),
    # so no prompts fire. We base64-encode to sidestep cmd.exe quoting.
    $psCmd = @'
